// intelhexfileio.cpp : Implementation der allgemeinen Klassen
//
...
THexFileMemRegion::THexFileMemRegion(int StartAddress, const QByteArray& byteArray)
{
    ...
    ByteArrContent.duplicate(byteArray);
}
...
THexFileMemRegion& THexFileMemRegion::operator = (const THexFileMemRegion& obj)
{
	...
	ByteArrContent.resize(obj.ByteArrContent.size());
	ByteArrContent.duplicate(obj.ByteArrContent);
	return *this;
}
...
#define BUFF_LEN 1024
	    ...
	    nByteRead = file.readBlock(FileBuffer, BUFF_LEN);
...
void cIntelHexFileIO::GetMemoryBlock(const int& nBlockLen, int& nStartAddressModuloBlockLen, QByteArray& byteArray, ulong& nOffsetToModulo)
{
        int nMaxAdress = nStartAddressModuloBlockLen + nBlockLen - 1;
	
	THexFileMemRegionList::iterator posNext;
	// Search regions affected
	THexFileMemRegionList AffectedMemRegions;
	//CList<POSITION, POSITION&> AffectedMemRegionPositions;
	for (posNext = m_ListMemRegions.begin(); posNext != m_ListMemRegions.end(); ++posNext)
	{
	    THexFileMemRegion& CurrEntry = *posNext;
//	    if( (nStartAddressModuloBlockLen >= CurrEntry.nStartAddress && nStartAddressModuloBlockLen <= CurrEntry.GetMaxAddress()) || (nMaxAdress >= CurrEntry.nStartAddress && nMaxAdress <= CurrEntry.GetMaxAddress()) ) // die startadresse bzw. die endadresse liegt innerhalb der region 
	...
	//	if (AffectedMemRegionPositions.count()) // wenn wir eine region gefunden haben
	//	    break; // abbruch
	    }
	...
	//	posNext = AffectedMemRegions.begin(); // die region haben wir ja schon
		...
		posNext = m_ListMemRegions.find(entry); // also nächste
		posNext++;
		while (posNext != m_ListMemRegions.end())
		{
		    THexFileMemRegion CurrEntry = *posNext;
		    
//		    if ( (nStartAddressModuloBlockLen >= CurrEntry.nStartAddress && nStartAddressModuloBlockLen <= CurrEntry.GetMaxAddress()) || (nMaxAdress >= CurrEntry.nStartAddress && nMaxAdress <= CurrEntry.GetMaxAddress()) )
...
bool cIntelHexFileIO::isHexText(const char c)
{
...
uchar cIntelHexFileIO::ASCII2Hex(char c)
{
    uchar uc;
    uc = c - 0x30;
    if (uc < 0xA)
